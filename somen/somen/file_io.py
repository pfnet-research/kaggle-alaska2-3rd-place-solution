...
def save_npz(datasets: Union[np.ndarray, Sequence[np.ndarray]], filepath: PathLike):
    _check_dir(filepath)
...
def load_npz(filepath: PathLike, allow_pickle=False) -> Union[np.ndarray, List[np.ndarray]]:
    _maybe_record_hash(filepath)
...
def save_json(X: dict, filepath: PathLike, *args, **kwargs) -> None:
    _check_dir(filepath)
...
DACITE_TYPE_HOOKS = {Path: lambda x: Path(x)}
DACITE_INV_TYPE_HOOKS = {Path: lambda x: str(x)}


def _apply_inv_type_hooks(data: Any) -> None:
...
def save_yaml_from_dataclass(X, filepath: PathLike) -> None:
    _check_dir(filepath)
    with open(str(filepath), "w") as fp:
...
def load_yaml_as_dataclass(
    ...
    if filepath is not None:
        ...
        result = dacite.from_dict(
            data_class=target_class, data=data, config=dacite.Config(type_hooks=DACITE_TYPE_HOOKS)
        )
    else:
        result = target_class()
